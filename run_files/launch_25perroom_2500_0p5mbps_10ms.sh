...
# Network params: 0.5 Mbps / 10 ms / pfifo, receive-only Zoom participants.
...
LOG_DIR="logs/scale25_0p5m10ms_${RUN_TAG}"
PID_DIR="pids/scale25_0p5m10ms_${RUN_TAG}"
mkdir -p "$LOG_DIR" "$PID_DIR"

for R in 1 2; do
  PORT=$(( 8015 + R ))
  for S in $(seq 1 $SLOTS); do
    RLOG="${LOG_DIR}/scale25_0p5m10ms_room${R}_slot${S}.log"
    RJL="${LOG_DIR}/scale25_0p5m10ms_room${R}_slot${S}.jsonl"
    ROOM_INDEX=$R SLOT_INDEX=$S SLOTS_PER_ROOM=$SLOTS NUM_ROOMS=$NUM_ROOMS \
      ...
      ZOOM_CAPACITY_MBPS=0.5 ZOOM_LATENCY_MS=10 ZOOM_QDISC=pfifo ZOOM_WAIT_SECONDS=30 \
      SWEEP_LOG="$RJL" MASTER_CTP_LIST="$MASTER" \
      setsid python3 -u room_receiver.py > "$RLOG" 2>&1 < /dev/null &
    PID=$!; disown $PID
    echo "$PID" > "${PID_DIR}/scale25_0p5m10ms_room${R}_slot${S}.pid"
    echo "room${R} slot${S} pid=${PID}"
    sleep $STAGGER
  done
done
echo "[launch] 50 receivers launched (25/room x 2 rooms) on 2500 list at 0.5Mbps/10ms"
