...
# master_cosine_2500[g::50] (~50 CTPs, no overlap). Orchestrator retries +
# phase-jitter are configured on the orchestrators; receivers keep defaults.
set -u
cd "$(dirname "$0")"
mkdir -p logs pids
...
echo "[launch] 50 receivers launched (25/room x 2 rooms) on 2500 list at 10Mbps/100ms"
