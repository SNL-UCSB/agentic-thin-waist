...
# Network params: 10 Mbps / 200 ms / pfifo, receive-only Zoom participants.
...
for R in 1 2; do
  ...
  for S in $(seq 1 $SLOTS); do
    RLOG="logs/scale25_10m200ms_room${R}_slot${S}.log"
    RJL="logs/scale25_10m200ms_room${R}_slot${S}.jsonl"
    ROOM_INDEX=$R SLOT_INDEX=$S SLOTS_PER_ROOM=$SLOTS NUM_ROOMS=$NUM_ROOMS \
      ...
      ZOOM_CAPACITY_MBPS=10 ZOOM_LATENCY_MS=200 ZOOM_QDISC=pfifo ZOOM_WAIT_SECONDS=30 \
      SWEEP_LOG="$RJL" MASTER_CTP_LIST="$MASTER" \
      setsid python3 -u room_receiver.py > "$RLOG" 2>&1 < /dev/null &
    PID=$!; disown $PID
    echo "$PID" > "pids/scale25_10m200ms_room${R}_slot${S}.pid"
    echo "room${R} slot${S} pid=${PID}"
    sleep $STAGGER
  done
done
echo "[launch] 50 receivers launched (25/room x 2 rooms) on 2500 list at 10Mbps/200ms"
