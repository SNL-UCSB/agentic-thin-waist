...
from netgent.src.registry.triggers.base import TriggerRegistry, trigger_registry
...
class ProgramController:
    ...
    def check(self, states: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    async def acheck(self, states: list[dict[str, Any]]) -> list[dict[str, Any]]:
        overall_start = time.perf_counter()
        matching_states: list[dict[str, Any]] = []

        for state in states:
            state_start = time.perf_counter()
            if await self._acheck_state(state):
                matching_states.append(state)
    ...
    def _check_state(self, state: Mapping[str, Any]) -> bool:
        ...
        for check in checks:
            ...
            result = self.registry.run(trigger_name, param=dict(params))
            logger.debug(
    ...
    async def _acheck_state(self, state: Mapping[str, Any]) -> bool:
        checks = state.get("checks", state.get("triggers", []))
        if not isinstance(checks, list):
            raise ValueError("State 'checks' must be a list")
        ...
        for check in checks:
            ...
            result = await self.registry.arun(trigger_name, param=dict(params))
            logger.debug(
                "Trigger '%s' returned %s for params=%s",
                trigger_name,
                result,
