...
def _make_ready_ctp_resolution(cluster: str = "cluster0") -> CtpResolution:
    ...
    ul_pcap = Path(f"/fake/ctp/ctp_100_cluster_incoming_6M/{cluster}_tree2_profile861.pcap")
    ...
    res.__class__ = type("_TestCtp", (CtpResolution,), {"ready": property(lambda self: True)})
    return res
...
def _poll_until_done(client: TestClient, orch_id: str, max_seconds: int = 10) -> dict[str, Any]:
    for _ in range(max_seconds * 10):
        ...
        if payload["status"] in ("complete", "failed", "partial"):
            return payload
        time.sleep(0.1)
    raise AssertionError(f"Orchestration {orch_id} did not finish within {max_seconds}s")
...
def _make_happy_clients() -> MagicMock:
    ...
    clients.generate_workflow.return_value = {"workflow_id": "wf-001", "status": "pending"}
    clients.get_workflow_status.return_value = {"workflow_id": "wf-001", "status": "completed"}
    clients.get_workflow_result.return_value = {"workflow_id": "wf-001", "status": "completed"}
    ...
    clients.shape_substrate.return_value = {
        "status": "shaped",
        "bottleneck_state": {"download_mbps": 20.0, "upload_mbps": 20.0, "latency_ms": 50.0},
        "applied_commands": [],
    }
    clients.capture_substrate.return_value = {
        "capture_id": "cap-001", "status": "started", "pcap_path": "/tmp/test.pcap",
    }
    clients.get_capture.return_value = {
        "capture_id": "cap-001", "status": "finished", "pcap_path": "/tmp/test.pcap", "exit_code": 0,
    }
    clients.stop_capture.return_value = {"capture_id": "cap-001", "status": "stopped"}
    clients.replay_substrate.return_value = {"replay_id": "rep-001", "status": "started"}
    ...
    clients.run_experiment.side_effect = lambda p: {
        "experiment_id": p["experiment_id"], "status": "pending", "spec": p,
    }
    ...
    clients.post_result.return_value = {
        "result_id": "res-001", "experiment_id": "e1", "status": "stored",
    }
...
def test_happy_path(client, monkeypatch):
    ...
    with patch("app.api.intent.IntentParser") as mock_parser_cls, \
         patch("app.engine.orchestration_workflow.DownstreamClients", return_value=mock_clients), \
         patch("app.engine.orchestration_workflow.resolve_ctp", return_value=ready_ctp), \
         patch("app.api.intent.DownstreamClients", return_value=mock_clients):
    ...
    assert "ctp_100_cluster_6M/cluster0_tree9_profile869" in replay_call[0][0]["ctp_file"]
...
def test_ctp_corpus_missing(client, monkeypatch):
    ...
    with patch("app.api.intent.IntentParser") as mock_parser_cls, \
         patch("app.engine.orchestration_workflow.DownstreamClients", return_value=mock_clients), \
         patch("app.engine.orchestration_workflow.resolve_ctp", return_value=missing_ctp), \
         patch("app.api.intent.DownstreamClients", return_value=mock_clients):
        mock_parser_cls.return_value.parse.return_value = _PARSED_INTENT
...
def test_telemetry_save_failure(client, monkeypatch):
    ...
    mock_clients.post_result.return_value = {
        "error": "telemetry_service_unavailable", "stored": False,
    }
    ready_ctp = _make_ready_ctp_resolution()
