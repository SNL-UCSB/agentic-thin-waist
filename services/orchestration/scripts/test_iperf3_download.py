"""
End-to-end smoke test: submit an iperf3 10 Mbps download intent to the
...
Usage (from repo root):
    python3 services/orchestration/scripts/test_iperf3_download.py

    # Custom orchestrator URL or container name:
    ORCH_URL=http://localhost:8005 python3 services/orchestration/scripts/test_iperf3_download.py
    ORCH_CONTAINER=orchestration python3 services/orchestration/scripts/test_iperf3_download.py

What happens:
    1. POST /intent  → orchestrator submits iperf3 download experiment intent
...
TARGET_SERVER_IP = "128.111.5.234"

INTENT = (
    f"Run iperf3 with 10 Mbps download on the server with IP {TARGET_SERVER_IP} "
    f"port 5201 for 30 seconds duration"
)
...
def submit_intent() -> str:
    banner("Step 1: Submitting iperf3 download intent")
    payload = {
        "intent": INTENT,
    ...
    print(f"  target server: {TARGET_SERVER_IP}")
...
def main() -> None:
    banner("Orchestration Manager — iperf3 10 Mbps Download Intent Test")
    print(f"  Target:        {ORCH_URL}")
    print(f"  Container:     {ORCH_CONTAINER}  (server-side logs streamed below)")
    print(f"  Server IP:     {TARGET_SERVER_IP}")
    print(f"  Download rate: 10 Mbps")
