"""
End-to-end smoke test: join a Zoom meeting with fake WAV audio AND fake MJPEG
video via the orchestration service.

Identical to test_orchestration_zoom_audio.py in every parameter, but uses the
run_zoom_av_workflow which adds a "Start Video" step so the fake MJPEG camera
is transmitted alongside the JFK speech mic:
  - SUBSTRATE_FAKE_AUDIO_FILE=/opt/substrate-audio/zoom-audio.wav  (WAV mic)
  - SUBSTRATE_FAKE_VIDEO_FILE=/opt/substrate-audio/zoom-video.mjpeg (MJPEG cam)
  Both are baked into the substrate-worker image and streamed via Chrome flags
  --use-file-for-fake-audio-capture and --use-file-for-fake-video-capture.

Usage (from repo root):
    python3 services/orchestration/scripts/test_orchestration_zoom_video.py
...
Override defaults via env vars:
    ...
    ZOOM_MEETING_ID     (default: 81179098491)
...
# Meeting parameters
MEETING_ID   = os.getenv("ZOOM_MEETING_ID",   "89597290818")
PASSCODE     = os.getenv("ZOOM_PASSCODE",     "046582")
...
def submit_intent() -> str:
    banner("Step 1: Submitting Zoom audio+video intent")
    payload = {
        "intent": INTENT,
        "context": {
            "application": "zoom",
        ...
        # Pin the audio+video workflow — local override in
        # config/workflows/run_zoom_av_workflow.json adds Join Audio + Start Video.
    ...
    print(f"  audio:        WAV streamed as fake mic  (SUBSTRATE_FAKE_AUDIO_FILE)")
    print(f"  video:        MJPEG streamed as fake cam (SUBSTRATE_FAKE_VIDEO_FILE)")
...
def main() -> None:
    banner("Orchestration — Zoom Meeting Join Test (audio + video)")
    ...
    if final_status == "complete":
        print("  ✓ Zoom audio+video experiment succeeded")
        sys.exit(0)
    else:
        print("  ✗ Zoom audio+video experiment did not complete successfully")
