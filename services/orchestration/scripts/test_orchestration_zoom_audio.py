"""
End-to-end smoke test: join a Zoom meeting with fake WAV audio via the
orchestration service.

Identical to test_orchestration_zoom.py in every parameter, but exercises the
audio path:
  - The ephemeral substrate-worker has SUBSTRATE_FAKE_AUDIO_FILE set to the
    baked-in WAV (/opt/substrate-audio/zoom-audio.wav), which Chrome streams
    as the browser microphone via --use-file-for-fake-audio-capture.
  - The local workflow override (services/orchestration/app/config/workflows/
    run_zoom_workflow.json) adds "Join Audio → Computer Audio" and unmute steps
    so the fake mic is actually transmitted into the meeting.

Usage (from repo root):
    python3 services/orchestration/scripts/test_orchestration_zoom_audio.py
...
# Meeting parameters
MEETING_ID   = os.getenv("ZOOM_MEETING_ID",   "83742755457")
PASSCODE     = os.getenv("ZOOM_PASSCODE",     "075508")
...
INTENT = (
    ...
    f"streaming WAV audio as the microphone. "
...
def submit_intent() -> str:
    banner("Step 1: Submitting Zoom + audio intent")
    payload = {
        "intent": INTENT,
        "context": {
            "application": "zoom",
        ...
        # Pin the workflow so the LLM doesn't have to guess; the local
        # override (config/workflows/run_zoom_workflow.json) will be used
        # because browser/agent.py checks local files before the remote repo.
    ...
    print(f"  audio:        WAV streamed as fake mic via SUBSTRATE_FAKE_AUDIO_FILE")
...
def main() -> None:
    banner("Orchestration — Zoom Meeting Join Test (with WAV audio)")
    ...
    print(f"  Workflow:     run_zoom_workflow (local override — joins audio + unmutes)")
    ...
    if final_status == "complete":
        print("  ✓ Zoom audio experiment succeeded")
        sys.exit(0)
    else:
        print("  ✗ Zoom audio experiment did not complete successfully")
