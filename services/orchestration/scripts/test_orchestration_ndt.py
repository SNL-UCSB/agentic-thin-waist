"""
End-to-end smoke test: submit an NDT speedtest intent to the orchestrator and
...
Usage (from repo root):
    ...
    ORCH_URL=http://localhost:8005 python3 services/orchestration/scripts/test_orchestration_ndt.py
    ORCH_CONTAINER=orchestration python3 services/orchestration/scripts/test_orchestration_ndt.py

What happens:
    1. POST /intent  → orchestrator submits NDT speedtest intent
...
def submit_intent() -> str:
    banner("Step 1: Submitting ndt speedtest intent")
    payload = {
        "intent": INTENT,
        "context": {"application": "ndt"},
        "preferences": {
...
def main() -> None:
    banner("Orchestration Manager — NDT Speedtest Intent Test")
