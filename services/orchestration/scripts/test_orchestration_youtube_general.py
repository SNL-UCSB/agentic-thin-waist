"""
End-to-end smoke test: submit a general YouTube playback intent to the
orchestrator and follow the pipeline live — including server-side print
statements streamed directly from the orchestration container logs.

Usage (from repo root):
    python3 services/orchestration/scripts/test_orchestration_youtube_general.py

    # Custom orchestrator URL or container name:
    ORCH_URL=http://localhost:8005 python3 services/orchestration/scripts/test_orchestration_youtube_general.py
    ORCH_CONTAINER=orchestration python3 services/orchestration/scripts/test_orchestration_youtube_general.py

Optional YouTube overrides:
    YOUTUBE_URL=https://www.youtube.com/watch?v=nXbHOUeFe8s
    WATCH_TIME=30
"""
...
WATCH_TIME = int(os.getenv("WATCH_TIME", "30"))
INTENT = (
    f"Test a youtube workflow using the URL {YOUTUBE_URL} and watch for "
    f"{WATCH_TIME} seconds, make capacity 200 Mbps and latency 10ms"
)
...
def submit_intent() -> str:
    banner("Step 1: Submitting general youtube playback intent")
    payload = {
        "intent": INTENT,
        "context": {"application": "youtube"},
        "preferences": {
...
def _verify_pcap(pcap_path: str, exp_id: str) -> None:
    ...
    if not pcap_path:
        print("    pcap: NOT recorded (pcap_path empty)")
        return
    ...
    print(
        "    pcap: path reported but not accessible from this host "
        "(worker already destroyed or path not mounted)"
    )
...
def main() -> None:
    ...
    print("  [orch]       = orchestration service (intent parse, dispatch, CTP)")
    print("  [worker/...] = ephemeral substrate worker (shape, run, capture)\n")
