...
| R10 | Disaggregated services, netUnicorn-style — but **logical disaggregation, physical packaging by profile** | Embrace netUnicorn's service decomposition (its Core ≈ mediation/compilation/infrastructure/gateway/processor maps onto Pramana's parser/KB+match/spec-gen/connectors/channel/scheduler). Module boundaries are enforced by the artifact-only interface rule and the must-not-know table (`pramana_interfaces_v0.md` §4) — never by co-residence. Physical packaging follows the profile: T1 runs the Core modules in one process (usability — six containers of Core on a laptop is netUnicorn's bloat failure), T2/T3 may deploy them separately. Disaggregate function, not operational units. |
