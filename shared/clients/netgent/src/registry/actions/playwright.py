...
async def click_element(
    ...
    # Best-effort mode: when True, a missing selector or a failed click is not
    # fatal — the action returns a "skipped" result so the workflow continues.
    # Used for UI elements that may or may not appear (e.g. Zoom's "Join Audio"
    # dialog, which varies by meeting/account configuration).
    is_optional = optional in (True, "true", "True", "1", "yes")
    ...
    if count == 0:
        if is_optional:
            return {"selector": selector, "clicked": False, "skipped": True}
